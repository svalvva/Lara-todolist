...
<div class="container py-4">
    ...
    <div class="row g-3 align-items-center mb-4">
        <div class="col-md-4">
            ...
            <p class="text-muted small mb-0">Kelola tugas-tugas Anda</p>
        </div>
        <div class="col-md-5">
                ...
                <li class="nav-item">
                    <a class="nav-link {{ request('status') === 'tertunda' ? 'active' : '' }}" 
                       href="{{ route('tasks.index', ['status' => 'tertunda']) }}">
                        ...
                        <span class="badge rounded-pill bg-primary ms-1">{{ $tasks->where('status', 'tertunda')->count() }}</span>
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {{ request('status') === 'selesai' ? 'active' : '' }}" 
                       href="{{ route('tasks.index', ['status' => 'selesai']) }}">
                        ...
                        <span class="badge rounded-pill bg-success ms-1">{{ $tasks->where('status', 'selesai')->count() }}</span>
                    </a>
                </li>
            </ul>
        </div>
                    ...
                    @foreach($tasks as $task)
                        <div class="list-group-item p-3 {{ $task->status == 'selesai' ? 'bg-light' : '' }}">
                            <div class="row align-items-center g-3">
                                ...
                                <div class="col">
                                    <h6 class="mb-1 {{ $task->status == 'selesai' ? 'text-decoration-line-through text-muted' : 'text-gray-900' }}">
                                        {{ $task->judul }}
                                    </h6>
                                    @if($task->deskripsi)
                                ...
                                <div class="col-auto">
                                    <div class="btn-group">
                                        <button class="btn btn-light btn-sm border shadow-none" 
                ...
                <form action="{{ route('tasks.update', $task->id) }}" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control" name="deadline" value="{{ $task->deadline->format('Y-m-d') }}">
                        </div>
                        <div class="mb-3">
